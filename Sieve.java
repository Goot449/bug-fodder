...
public class Sieve {
    ...
    /**
     * "Debug" version of PrintSieve.  This will show all numbers along with their T/F mark of
     * primality.
     *
    ...
    public static void printsieve2(int[] results, boolean[] prime) {
        System.out.print("> ");
        char temp = ' ';
        ...
        for (int j = 0; j < results.length; ) {
            temp = prime[j] ? 'T' : 'F';
            System.out.print(results[j] + "(" + temp + ") ");
            j++;
        }
        System.out.println();
    }
    ...
    /**
     * Prints out an array which should contain all prime values. Really, will print out any array.
     *
     ...
     * Get Array Size
     *
     * @param str the size of the array to return.
     * @return toReturn the size of the array.
     */

    public static boolean[] getTrueArray(int str) {
        boolean[] toReturn = new boolean[str];
        for (int j = 0; j < str; j++) {
            toReturn[j] = true;
        }
    ...
    /**
     * Convert our "behind the scenes" arrays - the list of ints from 1 to n and the same-size array
     * which indicates their primality - into a simple array of all prime values.
     *
    ...
    /**
     * Given an array of ints, starting at 1, calculate which ones are prime using the Sieve of
     * Eratosthenes. Uncomment println statements to watch it work.
     *
    ...
    /**
     * Generates our "off-by-one" array which will be all numbers that we calculate for the sieve.
     * That is, the zeroth element will contain the value 1, element 1 will contain the value 2,
     * etc.
     *
    ...
    /**
     * Main method.  Accepts one argument, which should be parseable as an integer, ignores any
     * other arguments.  This argument is the maximum value to calculate for the sieve. If no
     * argument is provided, or it cannot be parsed, assume the user meant 100.
     *
